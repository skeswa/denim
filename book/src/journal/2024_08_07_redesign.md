...
- Low-mental overhead
  ...
  - Aggressive complexity containment
    - [x] [`unknown` not `any`](#unknown-not-any)
...
let list: [i32]     = [1, 2, 3];
let set:  [:i32]    = [1, 2, 3]; // type decl determines type to be a set
...
```rust
let immutable_thing = (1, 2, 3);
// immutable_thing.0 = immutable_thing.0 + 1; // compile time error
let mut mutable_thing = (1, 2, 3);
...
print(mutable_thing); // Prints "(3, 4, 5)"

immutable_thing.inc(); // Compile time error

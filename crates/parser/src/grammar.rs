//! This is the actual "grammar" of the Denim language.
//!
...
/// The `parser` passed this is required to at least consume one token if it
/// returns `true`. If the `parser` returns false, parsing will stop.
fn delimited(
    p: &mut Parser<'_>,
    bra: SyntaxKind,
    ket: SyntaxKind,

...
use crate::{SyntaxKind, LAST_SYNTAX_KIND_TOKEN};

/// A bit-set of `SyntaxKind`s
#[derive(Clone, Copy)]
pub(crate) struct TokenSet([u64; 3]);

/// `TokenSet`s should only include token `SyntaxKind`s, so the discriminant of
/// any passed/included `SyntaxKind` must *not* be greater than that of the last
/// token `SyntaxKind`.
///
/// See https://github.com/rust-lang/rust-analyzer/pull/17037.
const LAST_TOKEN_KIND_DISCRIMINANT: usize = LAST_SYNTAX_KIND_TOKEN as usize;
...
fn token_set_works_for_tokens() {
    ...
    let ts = TokenSet::new(&[EOF, LAST_SYNTAX_KIND_TOKEN]);
    assert!(ts.contains(EOF));
    assert!(ts.contains(LAST_SYNTAX_KIND_TOKEN));
